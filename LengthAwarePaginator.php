...
class LengthAwarePaginator extends AbstractPaginator implements Arrayable, ArrayAccess, Countable, IteratorAggregate, Jsonable, JsonSerializable, LengthAwarePaginatorContract
{
    ...
    /**
     * Translate a pagination label, falling back to the given English text.
     *
     * Upstream guards this with function_exists('__'), which is always true
     * here — Venusian defines the helper regardless. Translation lands in
     * wave 4, so the guard has to ask whether a translator is actually bound.
     *
     * @param  string  $key
    ...
    /**
     * Get the array of elements to pass to the paginator's link collection.
     *
     * @return array
     */
    ...
    public function linkCollection()
    {
        return (new Collection($this->elements()))->flatMap(function ($item) {
            ...
            'label' => static::translate('pagination.previous', 'Previous'),
            'page' => $this->currentPage() > 1 ? $this->currentPage() - 1 : null,
            'active' => false,
        ])->push([
            'url' => $this->nextPageUrl(),
            'label' => static::translate('pagination.next', 'Next'),
